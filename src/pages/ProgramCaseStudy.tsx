...
const packets:Record<string,Packet> = {
  residual: {
    ...
    thesis:'I built Residual because agent systems get hard to trust when routing, retries, approvals, evidence, and verification all happen implicitly. I wanted those boundaries to be explicit enough that I could inspect and enforce them.',
    problem:'Once an agent system gets large enough, the hard part is not just getting a model to do the task. It is knowing what ran, what it was allowed to do, why a result was accepted, what happens when verification is uncertain, and where a human can still take control. Residual is my attempt to make those things part of the runtime instead of scattered conventions.',
    ...
    verification:['The automated test suite exercises receipt graphs, verifier identity, HITL replay prevention, routing boundaries, and extension behavior','Acceptance is kept separate from model confidence or provider output','Unknown and negative verifier states stay first-class outcomes instead of being quietly treated as success'],
    result:'The useful part is bigger than one agent workflow. I now have reusable pieces for contracts, receipts, verifiers, routing, durable state, and human escalation that can be dropped into other autonomous systems.',
